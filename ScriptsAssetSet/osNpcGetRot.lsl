...
and changes the state to hasNPC.
    In the hasNPC state:
        ...
        When the timer event triggers, the NPC says "Hello world!" and then retrieves its current rotation using osNpcGetRot() and says it.
...
default
{
    state_entry()
    {
        ...
        // instructing users to touch it to see the usage of osNpcGetRot.
...
state hasNPC
{
    ...
    timer()
    {
        ...
        // Make the NPC say its current rotation
